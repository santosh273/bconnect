...
/**
 * Generated class for the AsoPendingPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-aso-pending',
  templateUrl: 'aso-pending.html',
})
export class AsoPendingPage {
...
getCompleted(emdidx:any){
  ...
  this.http.get(Enums.APIURL.URL+'fetchadminsalesorderviewlist/'+emdidx+'/0').map(res => res.json()).subscribe(data => {
    // debugger;
...
viewDetails(Idx:string){
  this.navCtrl.push(ViewDetailsPage,{IDX:Idx,Aso:true});
}
newOrder(){
 
    this.navCtrl.push(MakeNewOrderPage,{Aso:true});
